/*************************************************************************
This peice of code has been created by Adheesh Gokhale for his project
Rectangling the panorama for CSE528. It contains of two sections. First
code developed by Adheesh. Second is code copied from
http://tipsandtricks.runicsoft.com/Cpp/BitmapTutorial.html
and has been marked so.
*************************************************************************/
#include <windows.h>
#include <stdio.h>
#include "CGProject.h"

//#inlcude "basic_modify.c"

//void doNothing1( BYTE* Buffer, int width, int height );
/*
void doSomething( BYTE* Buffer, int *width, int *height )
{

    //removeRed(Buffer, width, height );
    //markLargestBoundary(Buffer, *width, *height );
    //removeExtraBoundaries(Buffer, width, height );
    getGreyScale(Buffer, *width, *height );
}*/


//The code after this has been copied from http://www.runicsoft.com/bmp.cpp
//Courtesy: http://tipsandtricks.runicsoft.com/Cpp/BitmapTutorial.html
/*************************************************************************
...
BYTE* ConvertRGBToBMPBuffer1 ( BYTE* Buffer, int width, int height, long* newsize, int oldWidth )
{
...
BYTE* ConvertBMPToRGBBuffer ( BYTE* Buffer, int width, int height )
{
	...
	for ( int y = 0; y < height; y++ )
		for ( int x = 0; x < 3 * width; x+=3 )
		{
			...
			newbuf[newpos] = Buffer[bufpos + 2]; //red
...
void TestBMPCopy2(LPCTSTR input, char* output)
{
	int x, y, x1;
	long s, s2;
    ...
    BYTE* b11 = downsampling(b, &x,&y,4);
    printf("downsampling dimensions = %d , %d \n", x, y);
	BYTE* b1 = prepocess(b11,&x,&y);
    BYTE* c = ConvertRGBToBMPBuffer ( b1, x, y, &s2 );
	//SaveBMP ( c, x, y, s2, strcat(output,"_p.bmp") );
	output[14]='\0';
	printf("preprocessing dimensions = %d , %d \n", x, y);




	BYTE* b2 = LocalWarping( b1, x, y, output);
	b2 = LocalWarping( b2, x, y, output);

	BYTE* b3 = scaleup2( b2, &x, &y);
    b3 = scaleup2( b3, &x, &y);
    printf("upscaled dimensions = %d , %d \n", x, y);

	c = ConvertRGBToBMPBuffer ( b3, x, y, &s2 );
	SaveBMP ( c, x, y, s2, strcat(output,".bmp") );
	delete [] a;
	delete [] b;
	delete [] c;
	delete [] b1;
...
int main ()
{
	...
	strcpy(input, "images/input ");
	strcpy(output, "images/output ");
    printf("\n");


    for (char i = '1'; i<='1';i++){ //change the test condition from 1-9 for more files
	input[12]=i;
	input[13]='\0';
